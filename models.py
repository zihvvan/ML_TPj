...
def show_user_interface(model_num):
    ...
    나이 = r1_col1.slider("나이",20,70,key=keys[model_num])
    일일급여 = r1_col2.slider("일일급여", 110, 1500,key=keys[model_num])
    회사와의거리 = r1_col3.slider("회사와의거리", 1, 30,key=keys[model_num])
    근무환경만족 = r1_col4.slider("근무환경만족", 1, 4,key=keys[model_num])

    # 두번째 행
    r2_col1, r2_col2, r2_col3, r2_col4 = st.columns(4)
    시간당임금 = r2_col1.slider("시간당임금",30, 100,key=keys[model_num])
    직업만족도 = r2_col2.slider("직업만족도",1,4,key=keys[model_num])
    월수입 = r2_col3.slider('월수입',1000,20000,key=keys[model_num])
    이직회사수 = r2_col4.slider('이직회사수',0,9,key=keys[model_num])
    ...
    성별 = st.selectbox(
        '성별',
    ('남자', '여자'),key=keys[model_num])

    predict_button = st.button("퇴사유무 예측",key=keys[model_num])
